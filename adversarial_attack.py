...
for i, (img, _) in enumerate(data_loader):
    ...
    while 1:
        ...
        if(original_class != cl[0].item()):
            # not raising probability because we aim to make the adversarial attacking image.
            #thres_prob += 0.1
            break
        thres_prob -= 0.05

    # now here's an adversarial attacking image.
    # now, let's put this into the explainability method one more time.
    adversarial_image = explainability_masking(img[0].float(), explanations[i], p1=thres_prob)
    adversarial_saliency_maps = explainer(input_tensor=img[0].cuda().unsqueeze(0), targets=targets)
    tensor_imshow(img[0])
    plt.imshow(adversarial_saliency_maps[0], cmap='jet', alpha=0.5)
    plt.savefig("adversarial/adversarial_original_{:04d}.png".format(i))
    plt.close()
