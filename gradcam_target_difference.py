...
#args.datadir = '/home/seulgi/adversarial'
#args.datadir = '/home/seulgi/voice/images'
# Sets the range of images to be explained for dataloader.
args.range = range(95, 105)
#args.range = range(0, 19)
...
#explainer = GradCAM(model, args.input_size, args.gpu_batch)
...
if (EXPLAIN_FOR_THE_FIRST_TIME == 1):
    ...
    np.save('gradcam-exp_{:05}-{:05}.npy'.format(args.range[0], args.range[-1]), explanations)
else:
    explanations_filename = 'gradcam-exp_{:05}-{:05}.npy'.format(args.range[0], args.range[-1])
    explanations = np.load('/home/seulgi/nn-uncertainty/gradcam-exp_00000-00999.npy', allow_pickle=True)
...
for i, (img, data_classes) in enumerate(data_loader):
    ...
    for random_idx in range(100):
        ...
        model_output = model(randomly_masked_image.cuda().unsqueeze(0))

        chang_prob = torch.nn.functional.softmax(model_output, dim=1)
        ...
        if original_class == chang_class:
            ...
            explainability_list.append(saliency_maps[0])
    ...
    plt.savefig("result-target-diff/positive_gradcam-modelagnostic_add_{:04d}.png".format(i))
    plt.close()

    ############# image save for variance
    tensor_imshow(img[0])
    plt.imshow(variance, cmap='jet', alpha=0.5)
    plt.savefig("result-target-diff/positive_gradcampp-modelagnostic_variance_{:04d}.png".format(i))
    plt.close()
    ...
    plt.savefig("result-target-diff/positive_gradcampp-modelagnostic_originalexplainability_{:04d}.png".format(i))
    plt.close()
