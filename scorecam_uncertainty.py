...
from pytorch_grad_cam import ScoreCAM, GradCAMPlusPlus, AblationCAM, XGradCAM, EigenCAM, FullGrad
...
if USE_GRADCAM==1:
    ...
    explainer = ScoreCAM(model=model, target_layers=target_layers)
...
if (EXPLAIN_FOR_THE_FIRST_TIME == 1):
    ...
    np.save('exp_{:05}-{:05}_scorecam.npy'.format(args.range[0], args.range[-1]), explanations)
else:
    explanations_filename = 'exp_{:05}-{:05}_scorecam.npy'.format(args.range[0], args.range[-1])
    explanations = np.load('/home/sophia/nn-uncertainty/exp_00095-00104_scorecam.npy', allow_pickle=True)
...
for i, (img, _) in enumerate(data_loader):
    ...
    plt.savefig("scorecam_masks_point_2401220824/uncertainty_res_img_add_{:04d}.png".format(i))
    plt.close()

    ############# image save for variance
    tensor_imshow(img[0])
    plt.imshow(variance, cmap='jet', alpha=0.5)
    plt.savefig("scorecam_masks_point_2401220824/uncertainty_res_img_variance_{:04d}.png".format(i))
    plt.close()

    ############ image save for RISE
    tensor_imshow(img[0])
    plt.imshow(rise, cmap='jet', alpha=0.5)
    plt.savefig("scorecam_masks_point_2401220824/uncertainty_res_img_RISE_{:04d}.png".format(i))
    plt.close()

    ############ image save for entropy
    tensor_imshow(img[0])
    plt.imshow(entropy, cmap='jet', alpha=0.5)
    plt.savefig("scorecam_masks_point_2401220824/uncertainty_res_img_entropy_{:04d}.png".format(i))
    plt.close()

    ############ image save for entropy+RISE
    tensor_imshow(img[0])
    plt.imshow(entropy_for_RISE, cmap='jet', alpha=0.5)
    plt.savefig("scorecam_masks_point_2401220824/uncertainty_res_img_entropyRISE_{:04d}.png".format(i))
    plt.close()
