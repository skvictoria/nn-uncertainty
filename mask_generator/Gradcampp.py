...
explainer = GradCAMPlusPlus(model, target_layers=target_layers)
...
if (EXPLAIN_FOR_THE_FIRST_TIME == 1):
    ...
    np.save(explanation_dir+'/gradcampp-exp_{:05}-{:05}.npy'.format(args.range[0], args.range[-1]), explanations)
else:
    explanations_filename = explanation_dir+'/gradcampp-exp_{:05}-{:05}.npy'.format(args.range[0], args.range[-1])
