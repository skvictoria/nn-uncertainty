...
explainer = GradCAM(model, target_layers=target_layers)
uncertain = RISE(model, args.input_size, args.gpu_batch)
targets=[ClassifierOutputTarget(295)]
# Generate masks for RISE or use the saved ones.
maskspath = 'masks.npy'
uncertain_maskpath = '/home/seulgi/work/data/gradcammasks/uncertainmask.npy'
...
if (EXPLAIN_FOR_THE_FIRST_TIME == 1):
    ...
    np.save(explanation_dir+'/gradcam-exp_{:05}-{:05}.npy'.format(args.range[0], args.range[-1]), explanations)
else:
    explanations_filename = explanation_dir+'/gradcam-exp_{:05}-{:05}.npy'.format(args.range[0], args.range[-1])
...
for i, (img, data_classes) in enumerate(data_loader):
    ...
    if len(same_explainability_list) != 0:
        ...
        np.save(mask_dir+'/diff_uncertain_{:05}.npy'.format(i), same_variance)
    #############################################################################
